...
use Tolle\NovaFlexibleContent\Value\FlexibleCast;
...
By implementing the `HasFlexible` trait on your models, you can call the `flexible($attribute)` method, which will automatically transform the attribute's value into a fully parsed `Tolle\NovaFlexibleContent\Layouts\Collection`. Feel free to apply this `flexible()` call directly in your blade views or to extract it into an attribute's mutator method as shown below:
...
use App\PageBlocks;
use Tolle\NovaFlexibleContent\Flexible;
use Tolle\NovaFlexibleContent\Layouts\Preset;
...
It will place the new Resolver class in your project's `app/Nova/Flexible/Resolvers` directory. Each Resolver should implement the `Tolle\NovaFlexibleContent\Value\ResolverInterface` contract and therefore feature at least two methods: `set` and `get`.
 ...
 * @param  Tolle\NovaFlexibleContent\Layouts\Collection $layouts
...
2. Your custom layout class must implement `Spatie\MediaLibrary\HasMedia` and use the `Tolle\NovaFlexibleContent\Concerns\HasMediaLibrary` trait.
...
use Tolle\NovaFlexibleContent\Concerns\HasMediaLibrary;
